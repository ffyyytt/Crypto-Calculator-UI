...
void sha1::setState(int position, uint64 value)
{
	state[position] = value;
}

void sha1::setBuffer(int position, uint8 value)
{
	buffer[position] = value;
}

uint64 sha1::getState(int position)
{
	return state[position];
}

uint8 sha1::getBuffer(int position)
{
...
void sha1::transform(uint8 data[64])
{
...
void sha1::update(uint8* input, uint32 length)
{
	uint32 left, fill;
